...
public interface SchemaLeaseRepository extends JpaRepository<SchemaLease, Long> {
    ...
    @Query("select sl from SchemaLease sl "
        + "where sl.status = LeaseStatus.ACTIVE "
    ...
    @Query("select sl from SchemaLease sl " + "where sl.schemaName = :schemaName "
        + "and sl.jdbcUrl = :jdbcUrl " + "and sl.status = LeaseStatus.ACTIVE")
    Optional<SchemaLease> findActive(@Param("schemaName") String schemaName,
